...
@router.post("/customers/{customer_id}/plans/{plan_id}", tags=["Plans"], status_code=status.HTTP_201_CREATED)
async def subscribe_customer_to_plan(customer_id: int, plan_id: int, session: SessionDep):
    ...
    if not plan_db:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Plan doesn't exist")
...
@router.post("/customers/{customer_id}/plans/", tags=["Plans"])
